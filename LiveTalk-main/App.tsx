...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAnnouncements = onSnapshot(qAnnouncements, (snapshot) => {
      if (!snapshot.empty) {
        ...
        if (id !== lastAnnouncementId.current) {
          ...
          setTimeout(() => setAnnouncement(null), 6500);
        }
      }
    });
    ...
    const unsubGiftSettings = onSnapshot(doc(db, 'appSettings', 'gift_settings'), (snap) => {
      if (snap.exists()) {
        setGiftCategoryLabels(snap.data().categoryLabels);
      }
    });
    ...
    const unsubUsers = onSnapshot(collection(db, 'users'), (snapshot) => {
        ...
        if (user) {
          ...
          if (currentInDb) {
            const mergedUser = { ...currentInDb, ...pendingUserUpdates.current };
            setUser(mergedUser);
    ...
    return () => { 
      ...
      unsubAnnouncements(); unsubIdentity(); unsubGames(); unsubRooms(); unsubUsers(); unsubGifts(); unsubStore(); unsubVips(); unsubGiftSettings();
    };
  }, []);
