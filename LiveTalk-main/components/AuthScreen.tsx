...
const AuthScreen: React.FC<AuthScreenProps> = ({ onAuth, appLogo, authBackground, canInstall, onInstall }) => {
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#020617] flex flex-col items-center justify-center overflow-hidden font-cairo px-4 relative">
      {authBackground && <div className="absolute inset-0 bg-cover bg-center opacity-40 blur-[2px]" style={{ backgroundImage: `url(${authBackground})` }} />}
      <div className="absolute inset-0 bg-black/60 z-0"></div>

      <AnimatePresence>
        {showSplash && (
          <motion.div exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-center">
            <div className="w-24 h-24 bg-gradient-to-br from-amber-400 to-orange-600 rounded-[1.8rem] p-0.5 shadow-2xl">
              <img src={LOGO} className="w-full h-full object-cover rounded-[1.6rem]" />
            </div>
            <h1 className="mt-4 text-2xl font-black text-white tracking-widest">LIVETALK</h1>
          </motion.div>
        )}
      </AnimatePresence>

      <div className="w-full max-w-[360px] flex flex-col items-center gap-4 relative z-10">
        <div className="text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-600 rounded-2xl mx-auto mb-1 p-0.5 shadow-xl">
