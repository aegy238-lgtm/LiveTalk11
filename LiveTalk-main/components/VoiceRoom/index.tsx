...
const VoiceRoom: React.FC<any> = ({ 
  ...
  useEffect(() => {
    const unsubRoom = onSnapshot(doc(db, 'rooms', initialRoom.id), { includeMetadataChanges: true }, (snap) => {
      if (snap.exists()) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, { includeMetadataChanges: true }, (snapshot) => {
      const msgs = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data(), isPending: doc.metadata.hasPendingWrites }))
        .filter((msg: any) => {
  ...
  const executeGiftSendOptimistic = (gift: Gift, quantity: number, recipientIds: string[], isComboHit: boolean = false) => {
    ...
    // 2. تفعيل الأنميشن فوراً (أهم خطوة للسرعة)
    ...
    // 3. معالجة البيانات في "الخلفية" (Background Task)
    setTimeout(() => {
      let winAmount = 0;
      if (gift.isLucky) {
      ...
      // تحديث الحالة المحلية (Optimistic Update)
      onUpdateUser({ 
    ...
    }, 0); // تنفيذ فوري بعد فريم الأنميشن الأول
  ...
  const commitSingleGift = (gift: Gift, qty: number, recIds: string[], cost: number, win: number, speakers: any[]) => {
    try {
      ...
      recIds.forEach(rid => {
        batch.update(doc(db, 'users', rid), { charm: increment(cost / recIds.length), diamonds: increment(cost / recIds.length) });
        batch.set(doc(db, 'rooms', initialRoom.id, 'contributors', currentUser.id), {
          userId: currentUser.id, name: currentUser.name, avatar: currentUser.avatar, amount: increment(cost / recIds.length)
        }, { merge: true });
      });
      ...
      batch.commit().catch(e => console.error("Gift background commit failed", e));
  ...
  const handleClaimBag = async (bag: LuckyBag) => {
    ...
    try {
      ...
      updateDoc(doc(db, 'lucky_bags', bag.id), { 
        remainingAmount: increment(-share), 
        claimedBy: arrayUnion(currentUser.id) 
      }).catch(e => console.error("Bag claim sync failed", e));
    } catch (e) {}
  };
  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col bg-slate-950 font-cairo overflow-hidden text-right">
      ...
      <AnimatePresence>
        ...
        {activeBags.map(bag => (
           <LuckyBagActive 
             key={bag.id} bag={bag as any} isClaimed={bag.claimedBy.includes(currentUser.id)} onClaim={() => handleClaimBag(bag)} 
           />
        ))}
      </AnimatePresence>
      ...
      <div className="flex-1 relative flex flex-col overflow-hidden">
        ...
        <div className="h-64 px-4 mb-4 overflow-hidden relative z-[60]" dir="rtl">
           <div 
             ...
             className="h-full overflow-y-auto overflow-x-hidden scrollbar-hide space-y-4 flex flex-col pb-4 pointer-events-auto touch-pan-y"
